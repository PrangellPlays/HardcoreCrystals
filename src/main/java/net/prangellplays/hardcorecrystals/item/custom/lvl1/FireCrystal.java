...
public class FireCrystal extends Item {
    public FireCrystal(Settings settings) {
        super(settings);
    ...
    public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        if (!worldIn.isClient) {
            if (entityIn instanceof LivingEntity player) {
                if (player.getOffHandStack().isOf(ModItems.FIRE_CRYSTAL)) {
                    ((LivingEntity) entityIn).addStatusEffect(new StatusEffectInstance(StatusEffects.FIRE_RESISTANCE, 20, 1));
                }
            }
        }
    }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if (Screen.hasShiftDown()) {
            tooltip.add(Text.literal("Watch And Stare As The World Burns!").formatted(Formatting.RED));
        }   else {
